...
# Load the workbook
wb = openpyxl.load_workbook('ronec_dochadzka.xlsx', data_only=True)
sheet = wb.active
...
# Dynamically locate 'odpracovaný čas' column
hours_result = find_cell_by_text_partial(sheet, ['odpracovaný čas'])
if not hours_result:
    print("Column 'odpracovaný čas' not found.")
    exit()

hours_col = hours_result[1]
print(f"Found 'odpracovaný čas' at row {hours_result[0]}, column {get_column_letter(hours_col)}")

# Start extraction from two rows after the header row to skip extra header
...
# Export to CSV
with open('extracted_source.csv', 'w', newline='', encoding='utf-8') as csvfile:
...
print("Extraction completed. Data exported to 'extracted_source.csv'")